...
This example demonstrates how to use both Geertsma models (simple disc model
and full-grid model) to simulate surface subsidence due to reservoir pressure
...
4. Comparing the performance of the two models for different reservoir
   scenarios
"""
...
p_true_disc = (
    np.ones((1, 1)) * 15.0e6
)  # 15 MPa (pressure drop from initial 20 MPa)

# Calculate "true" subsidence
subsidence_true_disc = disc_model(p_true_disc)

# Add random noise to create synthetic observations
subsidence_obs_disc = subsidence_true_disc + rng.normal(
    0, dstd, size=subsidence_true_disc.shape
)
...
im1 = axs[0].pcolormesh(
    X_grid, Y_grid, Z_true_disc * 1000, cmap="RdBu_r", shading="auto"
)
...
im2 = axs[1].pcolormesh(
    X_grid, Y_grid, Z_obs_disc * 1000, cmap="RdBu_r", shading="auto"
)
...
print(f"Uncertainty reduction: "
      f"{(1 - p_post_std_disc / p_prior_std_disc) * 100:.1f}%")
error_reduction = (1 - abs(p_post_mean_disc - p_true_disc_value) /
                   abs(p_prior_mean_disc - p_true_disc_value)) * 100
...
# --------------------------------
#
# We'll create a spatially varying pressure field using a Gaussian random
# field.
...
# Add random noise to create synthetic observations
subsidence_obs_grid = subsidence_true_grid + rng.normal(
    0, dstd, size=subsidence_true_grid.shape
)
...
# True subsidence
im1 = axs[0].pcolormesh(
    X_grid, Y_grid, Z_true_grid * 1000, cmap="RdBu_r", shading="auto"
)
...
# Observed subsidence with noise
im2 = axs[1].pcolormesh(
    X_grid, Y_grid, Z_obs_grid * 1000, cmap="RdBu_r", shading="auto"
)
axs[1].set_title("Observed Subsidence (with noise)")
axs[1].set_xlabel("x (m)")
fig.colorbar(im2, ax=axs[1], label="Subsidence (mm)")
...
rmse_prior_subsidence = rmse(
    subsidence_prior_grid.mean(axis=0).reshape(nobs, nobs), Z_true_grid
)
rmse_post_subsidence = rmse(
    subsidence_post_grid.mean(axis=0).reshape(nobs, nobs), Z_true_grid
)

# Error reduction percentage
pressure_error_reduction = (1 - rmse_post_pressure / rmse_prior_pressure) * 100
subsidence_error_reduction = (
    1 - rmse_post_subsidence / rmse_prior_subsidence
) * 100

print("\nResults for Grid Model:")
print(
    f"RMSE in pressure field: Prior = {rmse_prior_pressure / 1e6:.2f} MPa, "
    + f"Posterior = {rmse_post_pressure / 1e6:.2f} MPa"
)
print(f"Pressure error reduction: {pressure_error_reduction:.1f}%")

print(
    f"RMSE in subsidence: Prior = {rmse_prior_subsidence * 1000:.2f} mm, "
...
ax.plot(
    x_cross, y_obs_cross_grid, "mo", alpha=0.5, label="Grid Model Observations"
)
...
#    - The disc model is simpler but can only estimate a single uniform
#      pressure
...
#
# The Geertsma models combined with ESMDA provide powerful tools for
# monitoring and managing geomechanical effects in various subsurface
