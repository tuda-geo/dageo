...
parameters when the pressure field is known but reservoir properties are
uncertain.
...
reservoir properties that control how this pressure change translates to
surface
...
for ax in axs:
    ax.add_patch(
        plt.Circle((0, 0), true_radius, fill=False, color="k", linestyle="--")
    )
    ax.set_aspect("equal")
...
# ----------------------------
#
# Generate an ensemble of parameter sets by sampling from the prior
# distributions.
...
thickness_prior = rng.uniform(
    thickness_bounds[0], thickness_bounds[1], size=ne
)
...
param_prior = np.column_stack(
    (depth_prior, radius_prior, thickness_prior, cm_prior, nu_prior)
)
param_names = [
    "Depth (m)",
    "Radius (m)",
...
for i, (name, ax) in enumerate(zip(param_names, axs)):
    ...
    ax.axvline(
        x=np.array(
            [true_depth, true_radius, true_thickness, true_cm, true_nu]
        )[i],
        color="red",
        linestyle="--",
    )
...
# ---------------------------
#
# Create a custom forward model function that takes parameter vectors and
# returns
...
for i in range(1, 6):
    ...
    im = axs[i].pcolormesh(
        X_grid, Y_grid, Z_prior * 1000, cmap="RdBu_r", shading="auto"
    )
...
def restrict_parameters(params):
    ...
    params[:, 2] = np.clip(
        params[:, 2], thickness_bounds[0], thickness_bounds[1]
    )
...
fig, axs = plt.subplots(
    2, 5, figsize=(15, 8), constrained_layout=True, sharex="col"
)
...
print("-" * 90)
print(f"{'Parameter':<15} {'True Value':<15} {'Prior Mean':<15} "
      f"{'Post Mean':<15} {'Error Red.(%)':<15} "
      f"{'Uncert. Red.(%)':<15}")
print("-" * 90)
...
im2 = axs[1].pcolormesh(
    X_grid, Y_grid, subsidence_prior_mean * 1000, cmap="RdBu_r", shading="auto"
)
...
im3 = axs[2].pcolormesh(
    X_grid, Y_grid, subsidence_post_mean * 1000, cmap="RdBu_r", shading="auto"
)
...
print(
    f"RMSE in subsidence: Prior = {rmse_prior * 1000:.3f} mm, "
...
# --------
#
# This example demonstrated how to use ESMDA to estimate Geertsma model
# parameters
# when the pressure field is known. The main findings are:
#
# 1. ESMDA can effectively estimate multiple reservoir parameters
#    simultaneously.
# 2. Some parameters are better constrained than others, depending on their
#    influence
