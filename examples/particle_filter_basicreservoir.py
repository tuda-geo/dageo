...
perm_true = RP(
    1, random=rng
)  # true log permeability field (shape: (1, nx, ny))
perm_prior = RP(
    n_particles, random=rng
)  # prior ensemble of log permeability fields (shape: (n_particles, nx, ny))
...
# Define simulation function that returns the pressure time series at well
# (ox, oy)
def sim(x):
    ...
    Returns:
      pressure time series : ndarray of shape (nt,), where nt = number of
        time points
    """
...
data_obs = true_pressure_series + rng.normal(
    0, dstd, size=len(true_pressure_series)
)
...
ax3.boxplot(
    [pred_prior[:, -1], pred_post[:, -1]], labels=["Prior", "Posterior"]
)
